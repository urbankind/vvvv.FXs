//@author:desaxismundi
//@help:bumpy,fresnel-shiny,dielectric,textured surface with two quadratic-falloff lights
//@tags:bump,gloss,fresnel,material
...
float KrMin <string UIName="Reflection Min";float UIMin=0.0;float UIMax=0.2;float UIStep=0.001;> =.05;
float FresExp <string UIName="Fresnel Exponent";float UIMin=1.0;float UIMax=7.0;> =5;
...
float4 ps(vs2ps IN):COLOR{
    ...
    float3 reflColor = fres*texCUBE(Samp3,float4(reflVect,1)).rgb;
    float3 result=(SurfCol.rgb*map*(Kd*diffContrib+AmbiCol.rgb))+specContrib+reflColor;
    return float4(result.rgb,1);
}

technique BumpyGlossy {pass p0 {VertexShader=compile vs_2_0 vs();PixelShader=compile ps_2_b ps();}}
