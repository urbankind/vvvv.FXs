...
// 3D version
float vnoise3D(float3 v,const uniform float4 pg[FULLSIZE])
{
    v = v + abs(v);
...
// 2D version
float vnoise2D(float2 v,const uniform float4 pg[FULLSIZE])
{
    v = v + abs(v);
...
// 1D version
float vnoise1D(float v,const uniform float4 pg[FULLSIZE])
{
    v = v + abs(v);
